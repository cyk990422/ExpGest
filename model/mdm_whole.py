...
class TextEncoder(nn.Module):
    def __init__(self, pretrained_model_name="/apdcephfs/share_1290939/shaolihuang/ykcheng/DiffCoSG/DiffuseStyleGesture/main/mydiffusion_zeggs/hub/models--bert-base-uncased/snapshots/1dbc166cf8765166998eff31ade2eb64c8a40076", output_dim=512):
...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats,
        ...
        checkpoint=torch.load("/apdcephfs/share_1290939/shaolihuang/iclr_ykcheng/DiffuseStyleGesture/main/mydiffusion_zeggs/final_output/expgest_sem_gest/vae_checkpoint_4400.bin")
        ...
        if 'style1' in self.cond_mode:
            ...
            if self.n_seed != 0:
                ...
                self.embed_text_body = nn.Linear(238 * n_seed, self.latent_dim)
                self.embed_text_hand = nn.Linear(756 * n_seed, self.latent_dim)
            else:
    ...
    def forward(self, x_body, x_hand, timesteps,hy_flag=False, y=None,uncond_info=False):
        ...
        if hy_flag==False:
            ...
            with torch.no_grad():
                text_latent_body,text_latent_hand=self.semges(wav_text)
            ...
            if 'cross_local_attention' in self.cond_mode:
                if 'cross_local_attention3' in self.cond_mode:
                    ...
                    # print("xseq_hand shape:",xseq_hand.shape)#182,1,512
                    # print("enc_text shape:",enc_text.shape)#180,1,64
                    zeros_pad = torch.zeros((2,enc_text.shape[1],64)).cuda()
                    enc_text_pad = torch.cat((zeros_pad,enc_text),axis=0)#182,1,64
                    xseq_hand =  torch.cat((xseq_hand,enc_text_pad),axis=-1)#182,1,512+64
