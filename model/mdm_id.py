...
#     def __init__(self, pretrained_model_name="/apdcephfs/share_1290939/zhengdiyu/take_away/ExpGes_repo/third_model/1dbc166cf8765166998eff31ade2eb64c8a40076", output_dim=256):
...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats,
        ...
        # checkpoint=torch.load("/apdcephfs/share_1290939/zhengdiyu/take_away/ExpGes_repo/main/mydiffusion_zeggs/final_output/train_sem_gest_align/vae_checkpoint_600.bin")
        ...
        if 'style2' not in self.cond_mode:
            self.input_process_body = InputProcess(self.data_rep, 238 + self.audio_feat_dim + self.gru_emb_dim, self.latent_dim)
            self.input_process_hand = InputProcess(self.data_rep, 756 + self.audio_feat_dim + self.gru_emb_dim, self.latent_dim)
        ...
        if self.arch == 'mytrans_enc':
            ...
            seqTransEncoderLayer_hand = nn.TransformerEncoderLayer(d_model=self.latent_dim + self.audio_feat_dim,
                                                              nhead=self.num_heads,
                                                              dim_feedforward=self.ff_size,
                                                              dropout=self.dropout,
                                                              activation=self.activation)
        ...
        if 'style1' in self.cond_mode:
            ...
            if self.n_seed != 0:
                self.style_dim = 64
                self.embed_style = nn.Linear(4, self.style_dim)
                #self.embed_text = nn.Linear(self.njoints * n_seed, self.latent_dim - self.style_dim)
                self.embed_text_body = nn.Linear(238 * n_seed, self.latent_dim - self.style_dim)
                self.embed_text_hand = nn.Linear(756 * n_seed, self.latent_dim - self.style_dim)
            else:
        ...
        self.output_process_body = OutputProcess(self.data_rep, 238, self.latent_dim, 238,
                                            self.nfeats)
        self.output_process_hand = OutputProcess(self.data_rep, 756, self.latent_dim + self.audio_feat_dim, 756,
                                            self.nfeats)
        ...
        if 'cross_local_attention' in self.cond_mode:
            ...
            self.rel_pos_hand_part2 = SinusoidalEmbeddings((self.latent_dim + self.audio_feat_dim) // self.num_head)
            #self.input_process = InputProcess(self.data_rep, self.input_feats + self.gru_emb_dim, self.latent_dim)
            self.input_process_body = InputProcess(self.data_rep, 238  + self.gru_emb_dim, self.latent_dim)
            self.input_process_hand = InputProcess(self.data_rep, 756  + self.gru_emb_dim, self.latent_dim)
            self.cross_local_attention_body = LocalAttention(
                ...
                window_size=20,  # window size. 512 is optimal, but 256 or 128 yields good enough results
    ...
    def forward(self, x_body, x_hand, timesteps,hy_flag=False, y=None,uncond_info=False):
        ...
        if hy_flag==False:
            ...
            text_embedding = self.embed_text(self.mask_cond(enc_text, force_mask=force_mask))
            #print("text_embedding shape:",text_embedding.shape,"     emb_t shape:",emb_t.shape)#1,bs,512

            ####从语义-手势联合分布中采样对齐后的自发语义对齐隐变量
            #训练时候开启
            ...
            # print("y['seed_body'] shape:",y['seed_body'].shape)
            # print("y['seed_body']:",y['seed_body'][0,13*6:13*6+3,0,:])
            if 'style1' in self.cond_mode:
                embed_style = self.mask_cond(self.embed_style(y['speaker_id']), force_mask=force_mask)       # (bs, 64)
                if self.n_seed != 0:
            ...
            if 'cross_local_attention' in self.cond_mode:
                if 'cross_local_attention3' in self.cond_mode:
                    ...
                    xseq_body = torch.cat((emb_1_body + emb_t,text_embedding.unsqueeze(1).permute(1, 0, 2), xseq_body), axis=0)
                    xseq_hand = torch.cat((emb_1_hand + emb_t, xseq_hand), axis=0)
                    # print("xseq_body shape:",xseq_body.shape)#182,1,512
                    # print("xseq_hand shape:",xseq_hand.shape)#181,1,512
                    # print("enc_text shape:",enc_text.shape)#180,1,64
                    zeros_pad = torch.zeros((1,enc_text.shape[1],64)).cuda()
                    enc_text_pad = torch.cat((zeros_pad,enc_text),axis=0)#181,1,64
                    xseq_hand =  torch.cat((xseq_hand,enc_text_pad),axis=-1)#181,1,512+64
                    ...
                    pos_emb_hand = self.rel_pos_hand_part2(xseq_hand)
                    xseq_body, _ = apply_rotary_pos_emb(xseq_body, xseq_body, pos_emb_body)
