...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x_body,x_hand, t,hy_flag, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        model_output_body,model_output_hand = model(x_body,x_hand, self._scale_timesteps(t),hy_flag, **model_kwargs)

        #Classifier-free guidence 
        
        model_output_body_uncond,model_output_hand_uncond = model(x_body,x_hand, self._scale_timesteps(t),hy_flag, **model_kwargs,uncond_info=True)
    ...
    def training_losses(self, model, x_start,x_start_body_joints,x_start_hand_joints, t,t_hy, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:     # LossType.MSE
            ...
            model_output_body,model_output_hand = model(x_t_body,x_t_hand,self._scale_timesteps(t), hy_flag=False,**model_kwargs)

            #get 滑步loss
            feats_rst_body=model_output_body.squeeze(2).permute(0,2,1)
            feats_rst_hand=model_output_hand.squeeze(2).permute(0,2,1)
            #[1, 1141, 1, 88]
            feats_rst_full=torch.zeros([feats_rst_body.shape[0],34,1141]).cuda()
