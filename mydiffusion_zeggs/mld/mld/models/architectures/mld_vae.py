...
class MldVae(nn.Module):
    ...
    def encode(
        ...
        #print("mask shape:",mask.shape)#128,34

        x = features
        #print("x shape:",x.shape)#128,34,282
        # Embed each human poses into latent vectors
        x = self.skel_embedding(x)
        #print("x1 shape:",x.shape)#128,34,256

        # Switch sequence and batch_size because the input of
        # Pytorch Transformer is [Sequence, Batch size, ...]
        x = x.permute(1, 0, 2)  # now it is [nframes, bs, latent_dim]
        #print("x2 shape:",x.shape)#34,128,256
        # Each batch has its own set of tokens
        dist = torch.tile(self.global_motion_token[:, None, :], (1, bs, 1))
        #print("dist shape:",dist.shape)#2,128,256
        # create a bigger mask, to allow attend to emb
        dist_masks = torch.ones((bs, dist.shape[0]),
                                dtype=bool,
                                device=x.device)
        #print("dist_masks shape:",dist_masks.shape)#128,2
        #print("mask shape:",mask.shape)#128,34
        aug_mask = torch.cat((dist_masks, mask), 1)
        #print("aug_mask shape:",aug_mask.shape)#128,36
        # adding the embedding token for all sequences
        xseq = torch.cat((dist, x), 0)
        #print("xseq shape:",xseq.shape)#36,128,256
        if self.pe_type == "actor":
            ...
            xseq = self.query_pos_encoder(xseq)
            #print("xseq encoder shape:",xseq.shape)#36,128,256
            dist = self.encoder(xseq,
                                src_key_padding_mask=~aug_mask)[:dist.shape[0]]
            #print("dist shape:",dist.shape)#2,128,256
