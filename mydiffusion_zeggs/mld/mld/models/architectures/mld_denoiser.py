...
from .tools.embeddings import (TimestepEmbedding,
                                                       Timesteps)
from ..operator import PositionalEncoding
from ..operator.cross_attention import (SkipTransformerEncoder,
                                                 TransformerDecoder,
...
class MldDenoiser(nn.Module):
    ...
    def __init__(self,
        ...
        self.linear=nn.Linear(self.text_encoded_dim+128, 256)
        ...
        # emb proj
        if self.condition in ["text", "text_uncond","audio"]:
            # text condition
    ...
    def forward(self,
                sample,#-1,1,256
                timestep,
                encoder_hidden_states,#-1*2,1,768   -1,34,256
                lengths=None,
                **kwargs):
        ...
        time_emb=time_emb.expand(time_emb.shape[0],34,time_emb.shape[2])

        text_emb = encoder_hidden_states  # -1,34,256
        # textembedding projection

        text_emb_latent = text_emb#-1,34,256
        fuse_feat_seq_now=torch.cat([sample,text_emb_latent],dim=2)#64,34,384
        #print("fuse_feat_seq_now shape:",fuse_feat_seq_now.shape)
        fuse_feat_seq = self.linear(fuse_feat_seq_now)#64,34,256
        x = time_emb + fuse_feat_seq#64,34,256
        xseq=self.query_pos(x)#64,34,256
        ...
        for blk in self.decoder_blocks:
            xseq = blk(xseq)
        xseq = self.decoder_norm(xseq)

        samples=self.out(xseq)
    ...
    def forward(self,
                sample,#-1,34,128
                timestep,
                encoder_hidden_states,#-1,1,256
                lengths=None,
                **kwargs):
        # 0.  dimension matching
        # sample [latent_dim[0], batch_size, latent_dim] <= [batch_size, latent_dim[0], latent_dim[1]]
        #bs,34,60
        sample = sample.permute(1, 0, 2)#34,-11,128
